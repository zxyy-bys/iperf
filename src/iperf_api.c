...
iperf_on_connect(struct iperf_test *test)
{
    ...
    if (test->role == 'c') {
        if (test->json_output)
            ...
            fprintf(stderr,"%s - %s %d: test->json_output == NULL\n", __FILE__, __FUNCTION__, __LINE__);
...
iperf_new_stream(struct iperf_test *test, int s, int sender)
{
    ...
    fprintf(stderr, "%s - %s %d: settings->blksize = %d\n", __FILE__, __FUNCTION__, __LINE__, test->settings->blksize);
    sp->buffer = (char *) malloc(test->settings->blksize * sizeof(char));
    ...
    /* fprintf(stderr, "%s - %s %d: sp->buffer = %s\n", __FILE__, __FUNCTION__, __LINE__, sp->buffer); */
